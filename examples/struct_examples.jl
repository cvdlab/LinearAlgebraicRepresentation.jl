...
structo=L.Struct([L.t(2,1),table,repeat([L.r(pi/2),chair],4)...])
structo1=L.Struct(repeat([structo,L.t(0,2.5)],10));
structo2=L.Struct(repeat([structo1,L.t(3,0)],10));
...
tablelegs = L.Struct( repeat([ tableLeg, L.r(0,0,pi/2) ],4) )
...
chairlegs = L.Struct( repeat([ chairLeg, L.r(0,0,pi/2) ],4) );
chair = L.Struct([ chairTop, chairlegs ]);
chair = L.struct2lar(chair)
View(chair)

theChair = L.Struct([ L.t(-.8,0,0), chair ])
fourChairs = L.Struct( repeat([L.r(0,0,pi/2), theChair],4) );
fourSit = L.Struct([fourChairs,table]);
View(fourSit)
singleRow=L.Struct(repeat([fourSit,L.t(0,2.5,0)],10));
View(singleRow)
refectory=L.Struct(repeat([singleRow,L.t(3,0,0)],10));
View(refectory)
